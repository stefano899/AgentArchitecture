...
evi(pronto):-sleep(15),write('Ciao a tutti, sono pronto!'),nl,nl,retract(ho_salutato(no)),assert(ho_salutato(si)).

eve(prontoag(var_Agente)):-write('Agente: '),write(var_Agente),write(' Pronto'),a(check(var_Agente)),nl.

richiesta_oggetto:-ho_salutato(si),ho_oggetto(no).

evi(richiesta_oggetto):-nl,nl,proprietario(var_P),write('Adesso richiedo l oggetto da mettere all asta al proprietario...'),nl,a(message(var_P,send_message(richiesta,var_Me))).

eve(oggetto):-nl,write('ho ricevuto oggetto'),nl,retract(ho_oggetto(no)),assert(ho_oggetto(si)).

decido_se_aprire:-ho_aperto(no),ho_salutato(si),ho_oggetto(si).

evi(decido_se_aprire):-random(0,4,var_A),assert(apro(var_A)),write(var_A),nl.

eve(sollecito):-nl,write('Vedo che la gente vuole spendere i propri soldi!'),nl,retract(apro(var_A)),assert(apro(1)),apro_offerte.

apro_offerte:-ho_aperto(no),ho_salutato(si),apro(var_A),var_A==1,ho_oggetto(si).

evi(apro_offerte):-nl,write('OFFERTE APERTE'),nl,retract(ho_aperto(no)),assert(ho_aperto(si)),a(invia_messaggio).

a(invia_messaggio):-a(message(bidder1,send_message(apertura,var_Me))),a(message(bidder2,send_message(apertura,var_Me))),a(message(bidder3,send_message(apertura,var_Me))).

eve(proposta(var_Bidder,var_Offerta)):-nl,write(var_Bidder),write(' ha offerto: '),write(var_Offerta),verifica_offerte(var_Bidder,var_Offerta),nl.

verifica_offerte(var_Bidder,var_Offerta):-controllo(var_Bidder,var_Offerta).

controllo(var_B,var_O):-nl,var_O>=30,write('Offerta accettata'),nl,a(message(var_B,send_message(accettata,var_Me))).

controllo(var_B,var_O):-nl,var_O<30,write('Offerta rifiutata'),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_361237,var_Ontology,_361241),_361231),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_361275)),a(message(var_Ag,propose(var_A,[_361275],var_AgI))),retractall(ext_agent(var_Ag,_361313,var_Ontology,_361317)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_361111,var_Ontology,_361115),_361105),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_361181,var_Ontology,_361185)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_360999,var_Ontology,_361003),_360993),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_361055,var_Ontology,_361059)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_360563),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_360597),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_360379).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_360227),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_360175),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_360051,_360053)),clause(agente(_360073,_360075,_360077,var_S),_360069),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_359845,_359847,_359849,var_S),_359841),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_359945,[]),repeat,read(_359945,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_359945).
