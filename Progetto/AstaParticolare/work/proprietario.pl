...
evi(pronto):-write('Ciao a tutti, sono il proprietario!'),nl,retract(ho_salutato(no)),assert(ho_salutato(si)),a(message(banditore,send_message(prontoag(var_Me),var_Me))).

eve(richiesta):-write('Invio oggetto da vendere'),nl,a(message(banditore,send_message(oggetto,var_Me))).

eve(richiesta_invio(var_Bidder)):-write('Ho ricevuto una richiesta di spedizione di un oggetto da parte di: '),write(var_Bidder),nl,write(' Per decidere se inviare o meno il pacco, lancero una monetina...'),nl,nl,lancio_monetina.

lancio_monetina:-random(0,2,var_M),var_Q is var_M,assert(monetina(var_Q)),write(var_M),nl,nl,(var_Q==1->write('E uscito testa, invio oggetto');write('E uscito croce, niente spedizione')),nl,nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_331767,var_Ontology,_331771),_331761),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_331805)),a(message(var_Ag,propose(var_A,[_331805],var_AgI))),retractall(ext_agent(var_Ag,_331843,var_Ontology,_331847)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_331641,var_Ontology,_331645),_331635),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_331711,var_Ontology,_331715)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_331529,var_Ontology,_331533),_331523),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_331585,var_Ontology,_331589)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_331093),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_331127),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_330909).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_330757),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_330705),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_330581,_330583)),clause(agente(_330603,_330605,_330607,var_S),_330599),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_330375,_330377,_330379,var_S),_330371),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_330475,[]),repeat,read(_330475,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_330475).
